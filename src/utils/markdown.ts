...
/**
 * Protect TeX inside $...$ / $$...$$ from markdown-it emphasis parsing.
 *
 * Problem: markdown-it will interpret `_x` / `_texture` as emphasis inside math
 * delimiters, injecting tags like <em> into the TeX stream, which then breaks
 * MathJax parsing on the client.
 *
 * Strategy: extract math spans to placeholders before markdown-it render, then
 * restore the original TeX (including delimiters) into the rendered HTML as
 * plain text, and let MathJax typeset it in the browser.
 */
const MJX_PLACEHOLDER_PREFIX = "@@MJX_MATH_";
const MJX_PLACEHOLDER_SUFFIX = "@@";

function extractMathForMathJax(markdown: string): { text: string; math: string[] } {
  const math: string[] = [];
  let text = markdown;

  // Block math first (allow newlines).
  text = text.replace(/\$\$([\s\S]*?)\$\$/g, (full) => {
    const idx = math.push(full) - 1;
    return `${MJX_PLACEHOLDER_PREFIX}${idx}${MJX_PLACEHOLDER_SUFFIX}`;
  });

  // Inline math (single-line). Keep this intentionally conservative.
  text = text.replace(/\$(?!\s)(?!\$)([^$\n]+?)\$/g, (full) => {
    const idx = math.push(full) - 1;
    return `${MJX_PLACEHOLDER_PREFIX}${idx}${MJX_PLACEHOLDER_SUFFIX}`;
  });
...
function restoreMathForMathJax(html: string, math: string[]): string {
  let out = html;
  for (let i = 0; i < math.length; i++) {
    const ph = `${MJX_PLACEHOLDER_PREFIX}${i}${MJX_PLACEHOLDER_SUFFIX}`;
    // Replace all occurrences (String.replace only replaces first).
    out = out.split(ph).join(math[i]!);
  }
  return out;
}
...
function renderNotes(html: string, notes: Note[]): string {
  ...
  for (let i = 0; i < notes.length; i++) {
    const note = notes[i];
    const { text: noteNoMath, math: noteMath } = extractMathForMathJax(note.content);
    let renderedContent = getMarkdownIt().render(noteNoMath);
    renderedContent = restoreMathForMathJax(renderedContent, noteMath);
...
export async function renderMarkdown(filePath: string): Promise<RenderedContent> {
  ...
  // Render markdown
  const { text: markdownNoMath, math } = extractMathForMathJax(markdownWithNotes);
  let html = getMarkdownIt().render(markdownNoMath);
  html = restoreMathForMathJax(html, math);
